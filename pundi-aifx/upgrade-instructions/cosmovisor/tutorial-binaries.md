...
{% hint style="info" %}
**Go 1.23+** or later is required for the Pundi AIFX. Install `go` by following the [official docs](https://golang.org/doc/install).
{% endhint %}
...
git clone https://github.com/PundiAI/fx-core.git
cd fx-core
git pull && git checkout release/v8.6.x
make build
```

```sh
mkdir -p $HOME/.fxcore/cosmovisor/upgrades/v8.6.x/bin
cp ./build/bin/fxcored $HOME/.fxcore/cosmovisor/upgrades/v8.6.x/bin/
...
5:27PM INF running app args=["version"] module=cosmovisor path=/root/.fxcore/cosmovisor/upgrades/v8.5.x/bin/fxcored
release/v8.5.3
```
